          ...
          {panelVisibility.properties && <aside className="properties-panel panel" tabIndex={0} aria-label="Properties panel">
            ...
            <section className="property-section">
              ...
              {activeLayer && <div className="layer-settings">
                ...
                {activeLayer.element === 'summary' && <label>Error bars<select value={activeLayer.errorBar ?? 'sd'} onChange={(event) => updateLayer(activeLayer.id, { errorBar: event.target.value as ErrorBarType })}><option value="sd">Sample SD</option><option value="se">Standard error</option><option value="ci95">95% confidence interval</option><option value="range">Range</option><option value="none">None</option></select><small>SD uses n − 1. Confidence intervals use Student's t and groups with n = 1 show no uncertainty.</small></label>}
