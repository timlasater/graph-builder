...
export function GraphCanvas() {
  const { dataset, spec, filters, updateSpec } = useBuilderStore()
  ...
  return <div className="chart-with-legend"><PlotlyChart data={data} layout={{ ...axes, autosize: true, title: { text: `<b>${spec.title}</b><br><span style="font-size:12px;color:#68737d">${spec.subtitle}${pageColumn ? ` · ${pageColumn.name}: ${String(spec.pageValue ?? pageValues[0] ?? '')}` : ''}</span>`, x: 0.04, xanchor: 'left' }, annotations, shapes, paper_bgcolor: '#ffffff', plot_bgcolor: '#fbfcfd', font: { family: 'Segoe UI, sans-serif', color: '#24313a', size: 12 }, margin: { l: groupYColumn ? 148 : 66, r: 24, t: groupXColumn ? 126 : 100, b: 54 }, showlegend: false, hovermode: 'closest', barmode: stackedBars ? 'stack' : 'group', bargap: 0.18 }} config={{ responsive: true, displaylogo: false, doubleClickDelay: 500, modeBarButtonsToRemove: ['sendDataToCloud', 'lasso2d'] }} /><div className="interactive-legend" role="list" aria-label="Graph series; drag to reorder"><span className="legend-help">Drag to reorder</span>{orderedLegendItems.map((item, index) => { const hidden = spec.hiddenSeries?.includes(item.id) ?? false; return <div className={`interactive-legend-item ${hidden ? 'hidden' : ''}`} role="listitem" tabIndex={0} draggable key={item.id} onDragStart={(event) => { event.dataTransfer.effectAllowed = 'move'; event.dataTransfer.setData('text/plain', item.id) }} onDragOver={(event) => { event.preventDefault(); event.dataTransfer.dropEffect = 'move' }} onDrop={(event) => { event.preventDefault(); reorderLegend(event.dataTransfer.getData('text/plain'), item.id) }} onKeyDown={(event) => { const targetIndex = event.key === 'ArrowLeft' ? index - 1 : event.key === 'ArrowRight' ? index + 1 : index; if (targetIndex !== index && orderedLegendItems[targetIndex]) { event.preventDefault(); reorderLegend(item.id, orderedLegendItems[targetIndex].id) } }}><span className="legend-grip" aria-hidden="true">⠿</span><input className="legend-color" type="color" value={item.color} aria-label={`Change color for ${item.label}`} title={`Change color for ${item.label}`} onChange={(event) => recolorLegend(item.id, event.currentTarget.value)} onBlur={(event) => recolorLegend(item.id, event.currentTarget.value)} onClick={(event) => event.stopPropagation()} draggable={false} /><button className="legend-label" aria-pressed={!hidden} title={`${hidden ? 'Show' : 'Hide'} ${item.label}`} onClick={() => toggleSeries(item.id)}>{item.label}</button></div> })}</div></div>
}
